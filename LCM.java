...
public class LCM {
    ...
    public static void main(String[] args) {
        try {
            ...
            HTTPServer server = new HTTPServer(1236);  // Exposes metrics on http://localhost:1236/metrics
            DefaultExports.initialize();

            // Initialize CloudSim and run the simulation
            ...
            // Create primary and secondary brokers
            DatacenterBroker primaryBroker = createBroker("PrimaryBroker");
            DatacenterBroker secondaryBroker = createBroker("SecondaryBroker");

            int primaryBrokerId = primaryBroker.getId();
            int secondaryBrokerId = secondaryBroker.getId();
            ...
            for (int i = 0; i < 10; i++) {
                Vm vm = new Vm(vmid, primaryBrokerId, mips, 1, ram, bw, size, vmm, new CloudletSchedulerTimeShared());
                vmlist.add(vm);
                vmid++;
            }

            primaryBroker.submitVmList(vmlist);
            secondaryBroker.submitVmList(vmlist); // Submitting VM list to secondary broker as well
            ...
            for (int i = 0; i < 20; i++) {
                ...
                cloudlet.setUserId(primaryBrokerId);
                cloudletList.add(cloudlet);
                id++;
            }

            primaryBroker.submitCloudletList(cloudletList);
            secondaryBroker.submitCloudletList(cloudletList); // Submitting cloudlet list to secondary broker as well

            // Implement Least Connection Load Balancer
            LeastConnectionLoadBalancer lc = new LeastConnectionLoadBalancer(primaryBroker, vmlist, secondaryBroker);
            lc.balanceLoad(cloudletList);

            // Start simulation
            CloudSim.startSimulation();

            // Monitor and migrate VMs based on load
            monitorAndMigrate(primaryBroker, secondaryBroker);
    ...
    static class LeastConnectionLoadBalancer {

        private DatacenterBroker primaryBroker;
        private DatacenterBroker secondaryBroker;
        private List<Vm> vmlist;

        public LeastConnectionLoadBalancer(DatacenterBroker primaryBroker, List<Vm> vmlist, DatacenterBroker secondaryBroker) {
            this.primaryBroker = primaryBroker;
            this.vmlist = vmlist;
            this.secondaryBroker = secondaryBroker;
        }

        public void balanceLoad(List<Cloudlet> cloudletList) {
            Map<Integer, Integer> vmLoadMap = new HashMap<>();

            // Initialize VM load map
            for (Vm vm : vmlist) {
                vmLoadMap.put(vm.getId(), 0);
            }

            // Assign each cloudlet to the VM with the least connections
            for (Cloudlet cloudlet : cloudletList) {
                int minLoadVmId = getMinLoadVm(vmLoadMap);
                cloudlet.setVmId(minLoadVmId);
                vmLoadMap.put(minLoadVmId, vmLoadMap.get(minLoadVmId) + 1);
            }
        }

        private int getMinLoadVm(Map<Integer, Integer> vmLoadMap) {
            return Collections.min(vmLoadMap.entrySet(), Map.Entry.comparingByValue()).getKey();
        }
    }

    private static void monitorAndMigrate(DatacenterBroker primaryBroker, DatacenterBroker secondaryBroker) {
        ...
        if (!vmsToMigrate.isEmpty()) {
            for (Vm vm : vmsToMigrate) {
                ...
                if (newHost != null) {
                    migrateVm(vm, newHost, primaryBroker, secondaryBroker);
    ...
    private static void migrateVm(Vm vm, Host newHost, DatacenterBroker primaryBroker, DatacenterBroker secondaryBroker) {
